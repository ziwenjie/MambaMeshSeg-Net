...
class OctreeMamba(nn.Module):
  ...
  def __init__(self, dim: int,
    ...
    self.pim = MambaMeshMix(input_dim=dim, output_dim=dim,fused_add_norm=True)
...
class MambaMeshBlock(torch.nn.Module):
...
class MambaMeshStage(torch.nn.Module):
...
class MambaMeshSegNet(torch.nn.Module):
  ...
  def __init__(self, in_channels: int,
    ...
    self.layers = torch.nn.ModuleList([MambaMeshStage(
        dim=channels[i],
...
class MambaMeshSegMix(nn.Module):
    def __init__(self,  
        output_dim=512,
        input_dim=512,
        drop_path = 0.1,
